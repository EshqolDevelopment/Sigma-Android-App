...
class MainActivity : AppCompatActivity() {
    ...
    private fun resultActivity(s: String) {
        ...
        DataBase().write("", "playing/$level/$groupCode/draw")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        quit.setOnClickListener {
            ...
            try {
                ...
                exit.setOnClickListener {
                    stop = true
                    DataBase().write("quit@$username", "playing/$level/$groupCode/winner$num")
        ...
        val tim = object : CountDownTimer((sec * 1000).toLong(), 1000) {
            ...
            override fun onFinish() {
                timer.text = "Times up"
                if (timeFinish && !stop) resultActivity("draw")
            }
        }
        tim.start()

        val myRef = database.getReference("playing/$level/$groupCode/winner$num")
        myRef.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                if (stop){
                    ...
                    if (data != "null") {
                        myRef.removeEventListener(this)
                        if (data == "quit@$player2"){
                            timeFinish = false
                            stop = true
                            resultActivity("quit")
                        }
                        else if (data == "quit@$username"){
                            "You canceled the game".show(this@MainActivity)
        ...
        val drawRef = database.getReference("playing/$level/$groupCode/draw")
        drawRef.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                if (stop){
                    drawRef.removeEventListener(this)
                }
                else{
                    val data = dataSnapshot.getValue<String>().toString()
                    ...
                    if (data == player2){
                        ...
                        snackbar = Snackbar.make(parentLayout, "", Snackbar.LENGTH_INDEFINITE)
                        ...
                        val accept = snackView.findViewById(R.id.txtTwo) as TextView
                        accept.setOnClickListener {
                            DataBase().write("draw", "playing/$level/$groupCode/winner$num")
                            snackbar.dismiss()
                        }

                        val reject = snackView.findViewById(R.id.txtOne) as TextView
                        reject.setOnClickListener {
                            DataBase().write("reject@$username", "playing/$level/$groupCode/draw")
                            snackbar.dismiss()
                        }
                    ...
                    else if (data == "reject@$player2"){
                        DataBase().write("", "playing/$level/$groupCode/draw")
                        snack.setText("The opponent rejected your offer")
                        thread {
                            Thread.sleep(2000)
                            snack.dismiss()
                        }
            ...
            if (lastPythonText != editText.text.toString() && textView.text.toString() != "Processing..."){
                ...
                try {
                    ...
                    if (textView.text.toString() == "True") {
                        ...
                        val myRef = database.getReference("playing/$level/$groupCode/winner$num")
                        myRef.setValue(username).addOnSuccessListener {
                            draw = true
                            timeFinish = false
                            resultActivity("won")
    ...
    override fun onDestroy() {
        super.onDestroy()
        if (!notQuit) DataBase().write("quit@$username", "playing/$level/$groupCode/winner$num")
        stop = true
    }
                ...
                if (!flagAlready){
                    ...
                    alert = AlertDialog.Builder(this)
                        ...
                        .setPositiveButton("Yes") { _, _ ->
                            try {
                                DataBase().write(username, "playing/$level/$groupCode/draw")
                                flagAlready = true
