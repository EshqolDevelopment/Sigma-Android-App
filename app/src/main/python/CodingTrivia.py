...
class Checker:
    def __init__(self, function, func_name, input_dict, output_dict, multi_arguments):
        ...
        self.input_dict = literal_eval(input_dict)
        self.output_dict = literal_eval(output_dict)
    ...
    def checks(self):
        ...
        forbidden_words = ["open", "os", "random", "__import__"]

        for word in forbidden_words:
            if word in self.function:
                self.result = "access denied"
                return "access denied"


        fVal = io.StringIO()
        with redirect_stdout(fVal):
            exec(self.function, d)
        self.log = fVal.getvalue()
